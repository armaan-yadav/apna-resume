...
export async function addSkillToResume(
  ...
  try {
    ...
    // Ensure skillDataArray is an array
    const skillsArray = Array.isArray(skillDataArray) ? skillDataArray : [];

    // Filter out entries with empty category, but allow empty skills array
    const validSkills = skillsArray.filter(
      (skill: any) => skill.category && skill.category.trim() !== ""
    );

    if (validSkills.length === 0) {
      // Clear all skills if none are valid
      resume.skills = [];
      await resume.save();
      revalidatePath(`/my-resume/${resumeId}/edit`);
      return { success: true, data: JSON.stringify(resume) };
    }

    const savedSkills = await Promise.all(
      validSkills.map(async (skillData: any) => {
        if (skillData._id) {
          const existingSkill = await Skill.findById(skillData._id);
          if (existingSkill) {
            return await Skill.findByIdAndUpdate(skillData._id, {
